...
const TeacherAuth = () => {
    ...
    const role = "teacher"; // Hardcoded to teacher
    ...
    const handleGoogleSignIn = async () => {
        ...
        try {
            ...
            if (!docSnap.exists()) {
                const finalName = name.trim() || user.displayName || "Unknown Teacher";
    ...
    return (
        <div className="flex-1 flex items-center justify-center -mt-10 lg:-mt-20">
            <div className="w-full max-w-md bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden sm:mx-0 mx-4">
                ...
                <div className="bg-slate-800 px-6 py-8 text-center sm:px-10">
                    <h2 className="text-2xl font-bold text-white mb-2">
                        {isLoginView ? "Teacher Portal Access" : "Teacher Registration"}
                    </h2>
                    <p className="text-slate-300 text-sm">
                        {isLoginView ? "Securely sign into the admin portal" : "Register a new teacher account"}
                    </p>
                </div>
                ...
                <div className="px-6 py-8 sm:px-10">
                    <form onSubmit={handleSubmit} className="space-y-6">
                            ...
                            <div>
                                ...
                                <div className="mt-1">
                                    <input
                                        ...
                                        className="w-full px-3 py-2 border border-gray-300 rounded-lg shadow-sm focus:ring-slate-800 focus:border-slate-800 sm:text-sm"
                        ...
                        <div>
                            <button
                                type="submit"
                                disabled={loading}
                                className="w-full flex justify-center py-2.5 px-4 border border-transparent rounded-lg shadow-sm text-sm font-medium text-white bg-slate-800 hover:bg-slate-900 disabled:opacity-50 transition-colors"
                            >
                                {loading ? "Loading..." : (isLoginView ? "Secure Sign In" : "Register Admin")}
                            </button>
                        </div>
                    ...
                    <div className="mt-8">
                        <button
                            ...
                            className="w-full flex justify-center py-2.5 px-4 rounded-lg text-sm font-medium text-slate-600 hover:bg-slate-100 transition-colors"
                        >
                            {isLoginView ? "Create new teacher account" : "Sign in to existing account"}
                        </button>
                    </div>
                </div>
            </div>
